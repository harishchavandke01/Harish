...
void BaselineProcessUI::setProjectContextData()
{
    ...
    for (auto it = posData.begin(); it != posData.end(); ++it)
    {
        ...
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 3; j++) {
                ...
                if (pd.cov_fixed[i][j] != 0.0) covValid = true;
            }
        if (!covValid) {
            double sigH = (pd.horizontalPrecision > 0) ? pd.horizontalPrecision : 0.010;
            double sigV = (pd.verticalPrecision  > 0) ? pd.verticalPrecision  : 0.020;
            for (int i = 0; i < 3; i++)
                for (int j = 0; j < 3; j++)
                    bl.cov[i][j] = 0.0;
            bl.cov[0][0] = sigH * sigH;
            bl.cov[1][1] = sigH * sigH;
            bl.cov[2][2] = sigV * sigV;
        }
        bl.rms = pd.RMS;
        bl.sigma0 = pd.sigma0;
        bl.dof = pd.dof;
    ...
    for (auto it = projectContext->stations.begin(); it != projectContext->stations.end(); ++it) {
        if (userFixedStationIds.contains(it.value().stationId)) it.value().isFixed = true;
    }
    chartView->drawChart(projectContext->stations, projectContext->baselines);
