...
NetworkAdjustment::NetworkAdjustment(ProjectContext *_projectContext, QWidget *parent)
    : QWidget(parent), projectContext(_projectContext)
{
    buildLeftPanel();
    chart = new QChart();
    chartView = new ChartView(chart);
...
void NetworkAdjustment::connectSignals()
{
    ...
    connect(adjustNetBtn,&QPushButton::clicked, this, &NetworkAdjustment::onAdjustNetClicked);
    connect(reportBtn, &QPushButton::clicked, this, &NetworkAdjustment::onReportClicked);
    connect(subnetCombo, QOverload<int>::of(&QComboBox::currentIndexChanged),
    ...
    if (projectContext) {
        ...
        connect(projectContext, &ProjectContext::adjustmentFinished, this, [this]() {
            ...
            const auto &ar = projectContext->adjustmentResult;
            if (!ar.mergedAdjustedECEF.isEmpty())
                chartView->drawChartAdjusted(
                    projectContext->stations,
                    projectContext->baselines,
                    ar.mergedAdjustedECEF);
            refreshButtonStates();
        });
    }
}
...
void NetworkAdjustment::buildLeftPanel()
{
    ...
    setControlsBtn->setObjectName("setBases");
    setControlsBtn->setCursor(Qt::PointingHandCursor);
    setControlsBtn->setToolTip(
        "Mark which stations hold known control coordinates.\n"
        "If none are checked, a Free Network adjustment runs.");
    ...
    adjustNetBtn->setCursor(Qt::PointingHandCursor);
    adjustNetBtn->setEnabled(false);
    adjustNetBtn->setToolTip(
        "Opens the Adjust Network dialog.\n"
        "Select which subnetworks to adjust and configure weighting.");
...
void NetworkAdjustment::buildStatsCard()
{
    ...
    statsTitle = new QLabel("Adjustment Results");
    statsTitle->setObjectName("networkStatsTitle");
...
void NetworkAdjustment::refreshModeBadge()
{
    ...
    if (fixed > 0) {
        modeBadge->setText("CONSTRAINED");
        modeBadge->setProperty("mode", "constrained");
        modeDesc->setText(QString("%1 control point%2 fixed")
                              .arg(fixed).arg(fixed == 1 ? "" : "s"));
    } else {
        modeBadge->setText("FREE NETWORK");
        modeBadge->setProperty("mode", "free");
...
void NetworkAdjustment::refreshSubnetCombo()
{
    ...
    for (auto it = ar.subnetworkResults.constBegin();
        ...
        const SubnetworkResult &r = it.value();
        QString label = QString("Subnetwork %1  (%2)")
                            .arg(r.subnetworkIndex)
                            .arg(r.constrained ? "Constrained" : "Free");
        subnetCombo->addItem(label, r.subnetworkIndex);
    }
...
void NetworkAdjustment::showStatsForSubnet(int subnetIndex)
{
    ...
    statType->setText(
        QString("Type: %1").arg(r.constrained ? "Constrained" : "Free Network"));
    statSigma0->setText(
        QString("σ₀: %1").arg(
            qIsNaN(r.sigma0) ? "—" : QString::number(r.sigma0, 'f', 4)));
    statDof->setText(
        QString("DOF: %1").arg(r.dof > 0 ? QString::number(r.dof) : "—"));
    statRms->setText(
        QString("RMS: %1").arg(
            qIsNaN(r.rms3D) ? "—"
                            : QString::number(r.rms3D * 1000.0, 'f', 1) + " mm"));
...
void NetworkAdjustment::onAdjustNetClicked()
{
    ...
    m_pendingSubnets    = toAdjust;
    m_currentSubnetJob  = 0;
    m_cancelRequested   = false;

    m_adjustProgressBar = new CustomProgressBar(m_pendingSubnets.size(), this);
    m_adjustProgressBar->setAttribute(Qt::WA_DeleteOnClose);
    m_adjustProgressBar->setModal(true);
    m_adjustProgressBar->show();

    connect(m_adjustProgressBar, &CustomProgressBar::rejected, this, [this]() {
        m_cancelRequested = true;
        m_adjustProgressBar->setStatus("Cancelling...");
    });

    startNextSubnetJob();
}

void NetworkAdjustment::startNextSubnetJob()
{
    if (m_cancelRequested) {
        m_adjustProgressBar->setStatus("Cancelled.");
        QTimer::singleShot(600, m_adjustProgressBar, &QWidget::close);
        return;
    }
    ...
    const int subnetIdx = m_pendingSubnets[m_currentSubnetJob];

    SubnetworkInfo targetInfo;
    bool found = false;
    for (const SubnetworkInfo &s : projectContext->subnetworks) {
        if (s.index == subnetIdx) { targetInfo = s; found = true; break; }
    }
    if (!found) {
        ++m_currentSubnetJob;
        startNextSubnetJob();
    ...
    m_adjustProgressBar->setStatus(
        QString("Adjusting subnetwork %1 of %2...")
            .arg(m_currentSubnetJob + 1)
            .arg(m_pendingSubnets.size()));

    QMap<QString, ProjectStation> subStations;
    for (const QString &uid : targetInfo.stationUIDs) {
        if (projectContext->stations.contains(uid))
            subStations.insert(uid, projectContext->stations[uid]);
    }
    ...
    RunNetworkAdjustment * m_currentWorker = new RunNetworkAdjustment(targetInfo, subStations, subBaselines, adjOptions);
    QThread *thread = new QThread();
    m_currentWorker->moveToThread(thread);

    connect(thread, &QThread::started, m_currentWorker, &RunNetworkAdjustment::ExecuteAdjust);

    connect(m_currentWorker, &RunNetworkAdjustment::statusUpdate,this, [this](const QString &msg) {
        m_adjustProgressBar->setStatus(msg);
    });

    connect(m_currentWorker, &RunNetworkAdjustment::finished,this, [this](SubnetworkResult result) {
        projectContext->adjustmentResult.storeSubnetworkResult(result);
        for (SubnetworkInfo &s : projectContext->subnetworks) {
            if (s.index == result.subnetworkIndex) {
                s.hasResult = result.success;
                break;
            }
        }
    ...
    connect(m_currentWorker, &RunNetworkAdjustment::failed,this, [this](int, const QString &reason) {
        m_adjustProgressBar->setStatus(QString("Subnetwork %1 failed: %2").arg(m_pendingSubnets[m_currentSubnetJob]).arg(reason));
        ++m_currentSubnetJob;
        startNextSubnetJob();
    });

    connect(thread, &QThread::finished, m_currentWorker, &QObject::deleteLater);
