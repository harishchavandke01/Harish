...
int NetworkAdjustment::fixedCount() const
{
    ...
    for (const auto &st : std::as_const(projectContext->stations))
        if (st.isFixed)  uniqueFixed.insert(st.stationId);
...
void NetworkAdjustment::startNextSubnetJob()
{
    ...
    m_adjustProgressBar->setStatus(QString("Adjusting subnetwork %1 of %2...") .arg(m_currentSubnetJob + 1) .arg(m_pendingSubnets.size()));
