#include "customloadingbar.h"

#include <QDebug>
#include <QtGlobal>
#include <QPoint>
#include <limits>
#include <cmath>

#define SPIN_INTERVAL 100

CustomLoadingBar::CustomLoadingBar(QWidget* parent) : QWidget(parent) {

    _type     = CustomLoadingBar::line_rotate;
    _interval = SPIN_INTERVAL;
    _angle    = 0;
    _scale    = 0.0f;
    _color    = Qt::black;

    setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed);
    setFocusPolicy(Qt::NoFocus);

    _timer = new QTimer();

    connect(_timer, SIGNAL(timeout()), this, SLOT(onTimeout()));
}

CustomLoadingBar::~CustomLoadingBar() {
    stop();
    delete _timer;
}

void CustomLoadingBar::paintEvent(QPaintEvent* e) {
    Q_UNUSED(e)
    if (!_timer->isActive()) {
        return;
    }
    QPainter painter(this);
    painter.setRenderHint(QPainter::Antialiasing);

    switch (_type) {
    case CustomLoadingBar::line_rotate:
        drawRotateLine(&painter);
        break;
    case CustomLoadingBar::line_scale:
        drawScaleLine((&painter));
        break;
    case CustomLoadingBar::ball_rotate:
        drawRotateBall(&painter);
        break;
    case CustomLoadingBar::circle_rotate:
        drawRotateCircle(&painter);
        break;
    case CustomLoadingBar::horizontal_infinite:
        drawHorizontalInfinite(&painter);
        break;
    }
}

void CustomLoadingBar::start() {
    _timer->start(_interval);
}

void CustomLoadingBar::stop() {
    _timer->stop();
    update();
}

int CustomLoadingBar::type () {
    return _type;
}

void CustomLoadingBar::setType (int type) {
    _type = type;
}

const QColor& CustomLoadingBar::color () {
    return _color;
}

void CustomLoadingBar::setColor (const QColor& color) {
    _color = color;
}

int CustomLoadingBar::interval () {
    return _interval;
}

void CustomLoadingBar::setInterval (int interval) {
    _interval = interval;
}

void CustomLoadingBar::onTimeout() {
    switch (_type) {
    case line_rotate:
    case ball_rotate:
        _angle = (_angle + 45) % 360;
        break;
    case circle_rotate:
        _angle = (_angle + 45) % 360;
        break;
    case line_scale:
        _scale += 0.1f;
        _scale = _scale > .5f ? 0.0f : _scale;
        break;
    case horizontal_infinite:
        _angle += 10.0f;
        if (_angle > width()) _angle = 0;
        break;
    }
    update();
}

void CustomLoadingBar::drawRotateLine(QPainter* painter) {
    int width = qMin(this->width(), this->height());

    int outerRadius = (width - 4) * 0.5f;
    int innerRadius = outerRadius * 0.42f;

    int capsuleHeight = outerRadius - innerRadius;
    int capsuleWidth  = (width > 32 ) ? capsuleHeight * .32f : capsuleHeight * .40f;
    int capsuleRadius = capsuleWidth / 2;

    for (int i = 0; i < 8; i++) {
        QColor color = _color;

        color.setAlphaF(1.0f - (i / 8.0f));
        painter->setPen(Qt::NoPen);
        painter->setBrush(color);

        painter->save();

        painter->translate(rect().center());
        painter->rotate(_angle - i * 45.0f);

        painter->drawRoundedRect(-capsuleWidth * 0.5, -(innerRadius + capsuleHeight), capsuleWidth, capsuleHeight, capsuleRadius, capsuleRadius);

        painter->restore();
    }
}

void CustomLoadingBar::drawScaleLine(QPainter* painter) {
    int height = qMin(this->width(), this->height());

    qreal lineWidth  = height * 0.15f;
    qreal lineHeight = height * 0.9f;
    qreal lineRadius = lineWidth / 2.0f;
    qreal lineGap    = lineWidth;
    qreal margin     = (this->width() - lineWidth * 5 - lineGap * 4) / 2.0f;

    for (int i = 0; i < 5; i++) {

        painter->setPen(Qt::NoPen);
        painter->setBrush(_color);

        int tmp = _scale * 10 + i + 1;
        if (tmp > 5) {
            tmp = 5 - tmp % 5;
        }

        qreal scale = 0.5f + tmp * 0.1f;
        qreal h = lineHeight * scale;

        painter->save();

        painter->translate(QPointF(margin + (lineWidth + lineGap) * i, this->height() / 2));

        painter->drawRoundedRect(0, -h / 2.0f, lineWidth, h, lineRadius, lineRadius);

        painter->restore();
    }
}

void CustomLoadingBar::drawRotateBall(QPainter* painter) {
    int width = qMin(this->width(), this->height());

    int outerRadius = (width - 4) * 0.5f;
    int innerRadius = outerRadius * 0.78f;

    int capsuleRadius = (outerRadius - innerRadius) / 2;

    for (int i = 0; i < 8; i++) {

        QColor color = _color;

        color.setAlphaF(1.0f - (i / 8.0f));

        painter->setPen(Qt::NoPen);
        painter->setBrush(color);

        qreal radius = capsuleRadius * (1.0f - (i / 16.0f));

        painter->save();

        painter->translate(rect().center());
        painter->rotate(_angle - i * 45.0f);

        QPointF centre = QPointF(-capsuleRadius, -(innerRadius + capsuleRadius));
        painter->drawEllipse(centre, radius * 2, radius * 2);

        painter->restore();
    }
}

void CustomLoadingBar::drawRotateCircle(QPainter* painter) {
    int width    = qMin(this->width(), this->height());
    int penwidth = 5;

    QRectF r = QRect(0+penwidth,0+penwidth, width-(2*penwidth),width-(2*penwidth));

    QPen pen(_color);
    pen.setWidth(penwidth);
    pen.setCapStyle(Qt::RoundCap);

    painter->setPen(pen);
    painter->setBrush(_color);

    int startAngle = 0.0;
    int spanAngle  = 45 * 16;

    startAngle = (0.0 + _angle) * 16;
    painter->drawArc(r, -startAngle, -spanAngle);

    startAngle = (120.0 + _angle) * 16;
    painter->drawArc(r, -startAngle, -spanAngle);

    startAngle = (240.0 + _angle) * 16;
    painter->drawArc(r, -startAngle, -spanAngle);
}

void CustomLoadingBar::drawHorizontalInfinite(QPainter* painter) {
    int w = width();
    int h = height();

    painter->setRenderHint(QPainter::Antialiasing);

    painter->setPen(QPen(Qt::gray, 2));
    painter->setBrush(Qt::NoBrush);
    QRect outerRect(1, 1, w - 2, h - 2);
    painter->drawRect(outerRect);

    int beamWidth = w / 4;
    int beamHeight = h - 4;

    float x = _angle;

    painter->setPen(Qt::NoPen);
    painter->setBrush(_color);
    QRectF beamRect(x, 2, beamWidth, beamHeight);
    painter->drawRoundedRect(beamRect, 3, 3);

    if(x + beamWidth > w) {
        QRectF wrapRect(x - w, 2, beamWidth, beamHeight);
        painter->drawRoundedRect(wrapRect, 3, 3);
    }
}
