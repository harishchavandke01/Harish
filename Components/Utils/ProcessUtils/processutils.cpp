...
bool ProcessUtils::parsePosFileIntoPosData(const QString &posFilePath, PosData &out, QString &errorOut)
{
    ...
    while (!in.atEnd()) {
        ...
        if (line.startsWith('%')) {
            ...
            if(line.contains("Baseline covariance", Qt::CaseInsensitive)){
                inCovBlock = true;
                covRow = 0;
                continue;
            }

            if(inCovBlock && line.contains("dX") && line.contains("dY")){
                continue;
            }

            if(inCovBlock && covRow<3){
                QString dataLine = line.mid(1).trimmed();
                QStringList vals = dataLine.split(' ',Qt::SkipEmptyParts);
                if(vals.size()>=3){
                    out.cov_fixed[covRow][0] = vals[0].toDouble();
                    out.cov_fixed[covRow][1] = vals[1].toDouble();
                    out.cov_fixed[covRow][2] = vals[2].toDouble();
                    covRow++;
                    if(covRow==3) inCovBlock = false;
                }
                continue;
            }
            ...
            if (m.hasMatch()) {
                ...
                if(key.compare("sigma0", Qt::CaseInsensitive) ==0 ){
                    out.sigma0 = val.toDouble();
                }
                if(key.compare("DOF", Qt::CaseInsensitive)==0){
                    out.dof = val.toDouble();
                }
