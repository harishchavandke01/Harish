...
// {
//     qDebug() << "Weight block for baseline:" << bl.fromStationId << "->" << bl.toStationId;
//     qDebug() << "Covariance:";
//     qDebug() << bl.cov[0][0] << bl.cov[0][1] << bl.cov[0][2];
//     qDebug() << bl.cov[1][0] << bl.cov[1][1] << bl.cov[1][2];
//     qDebug() << bl.cov[2][0] << bl.cov[2][1] << bl.cov[2][2];
...
//         qDebug() << "Misclosure for baseline" << bl.from << "->" << bl.to;
//         qDebug() << w(row) << w(row+1) << w(row+2);
...
void LSSolver::filterBaselines()
{
    ...
    for (const ProjectBaseline &bl : m_baselines) {
        ...
        QString key = fromC + "_" + toC;
        QString key_rev = toC + "_" + fromC;

        if (seen.contains(key) || seen.contains(key_rev)) continue;
