...
LSSolver::LSSolver(const SubnetworkInfo &info, const QMap<QString, ProjectStation> &stations, const QVector<ProjectBaseline> &baselines, const AdjustmentOptions &options)
    : m_info(info), m_stations(stations), m_baselines(baselines), m_options(options)
{
    buildCanonicalMap();
    initPositions();
    filterBaselines();
...
bool LSSolver::runInnerLoop(Eigen::VectorXd &dx_final, Eigen::MatrixXd &N_inv, Eigen::MatrixXd &A_final, Eigen::VectorXd &w_final, Eigen::MatrixXd &P_final, int outerIter, int &iterations)
{
    if (m_nFree == 0) {
...
void LSSolver::propagatePrecisions(const Eigen::MatrixXd &N_inv, double sigma0_sq, SubnetworkResult &result) const
{
    for (auto it = m_freeIdx.constBegin(); it != m_freeIdx.constEnd(); ++it) {
        ...
        sp.horizPrecision95 = SCALE_95_2D * semiMajor;
        sp.vertPrecision95 = SCALE_95_1D * sigU;
...
SubnetworkResult LSSolver::solve()
{
    ...
    for (int outer = 0; outer < MAX_OUTER; ++outer) {
        ...
        bool converged = runInnerLoop(dx_final, N_inv, A_final, w_final, P_final, outer + 1, result.iterations);
        if (!converged)
